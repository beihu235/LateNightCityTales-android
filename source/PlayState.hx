...
class PlayState extends MusicBeatState
{
	...
	public var evilTrailEX:FlxTrail;
...
//-----------------------------------------
	...
	var noteFiltersRed:Array<BitmapFilter> = [];
	...
	public var shaderBlurUpdates:Array<Float->Void> = [];
	public var blurFactor:Float = 0.0;
	public var blurAmount:Float = 0;

	public var shaderBloomUpdates:Array<Float->Void> = [];
	public var bloomFactor:Float = 0.0;
	public var bloomAmount:Float = 0;


	public var shaderGlitchUpdates:Array<Float->Void> = [];
	public var glitchFactor:Float = 1.0;
	public var glitchAmount:Float = 0;

	public var shaderCAUpdates:Array<Float->Void> = [];
	public var caAmountFactor:Float = 1.0;
	public var caAmount:Float = 0;
...
//----------------------------------------------------------------
		...
		camNoteRed.setFilters(noteFiltersRed);
...
//----------------------------------------------------------------
		...
		switch(curStage)
		{
			...
			case 'LimuCastle':
					//evilTrailEX = new FlxTrail(dad, null, 4,0, 0.3, 0.005); //nice
					//evilTrailEX = new FlxTrail(dad, null, 0, 60, 0.3, 0.001); //nice
					//insert(members.indexOf(dadGroup) - 1, evilTrailEX);
		}
		...
		var glowRed = new GlowFilter(FlxColor.RED,specialNoteGlowAlpha,specialNoteGlowBlur,specialNoteGlowBlur,specialNoteGlowStrength,3,false,false);
	...
	override public function update(elapsed:Float)
	{
		...
		if(caAmount>0){
			...
			if(caAmount<=0){caAmount=0;}
		}

		if(glitchAmount>0){
			glitchAmount-= (elapsed*glitchFactor);
			if(glitchAmount<0){glitchAmount =0;}
		}

		if(blurAmount>0){
			blurAmount-= (elapsed*blurFactor);
			if(blurAmount<0){blurAmount =0;}
		}

		if(bloomAmount>0){
			bloomAmount-= (elapsed*bloomFactor);
		...
		switch (curStage)
		{
			...
			case 'LimuCastle':
				// evilTrailEX.color = 0x7E2FFF00;

				// evilTrailEX.forEach(function(single:FlxSprite){
				// 	single.y-=2;
				// 	single.scale.set(single.scale.x+0.005,single.scale.y+0.005);
				// 	single.alpha-=0.01;
				// });
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				}else if(dunceNote.noteType=="red"){
					dunceNote.camera = camNoteRed;
				}
	...
	function updateBlur(amount:Float){
		blurAmount = amount;
	}
